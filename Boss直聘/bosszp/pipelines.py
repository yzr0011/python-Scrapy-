...
from bosszp import settings


class YBmysql(object):
    def __init__(self):
        dbparams = {
            'host': '127.0.0.1',
            'user': 'root',
            'password': '*********************************************',
            'database': 'bosszp',  ##数据库名
            'port': 3306,  ##端口号
            'charset': 'utf8',  ##编码格式
    ...
    def sql(self):
        if not self._sql:  ###如果没有_sql这个对象  就创建
            self._sql = """
                insert into sxs(id,name,gs,nr,url) values(null,%s,%s,%s,%s)
                """  ###id是设置自动增长的  传入null
            return self._sql
    ...
    def insert_item(self, cursor, item):  ####传入游标及item的对象       “插入item”函数
        cursor.execute(self.sql, (item['name'],item['gs'],item['nr'],item['url']))

    def handle_error(self, error, item,spider):  ####首先会有错误参数，别的在上面传入上面addErrback传入什么参数这里就有什么参数   item,spider在处理什么item和什么爬虫时出现问题
